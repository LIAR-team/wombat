...
class LocalSLAMTest : public TestKennelSingleRobot
{
public:
...
class WallsMapLocalSLAMTest : public LocalSLAMTest
{
public:
  void SetUp() override
  {
...
TEST_F(WallsMapLocalSLAMTest, DriveAndExplore)
{
  ...
  ASSERT_EQ(gt_map->info.height, local_map->info.height);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.position.x, local_map->info.origin.position.x);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.position.y, local_map->info.origin.position.y);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.position.z, local_map->info.origin.position.z);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.orientation.x, local_map->info.origin.orientation.x);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.orientation.y, local_map->info.origin.orientation.y);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.orientation.z, local_map->info.origin.orientation.z);
  ASSERT_DOUBLE_EQ(gt_map->info.origin.orientation.w, local_map->info.origin.orientation.w);
  ASSERT_EQ(gt_map->data.size(), local_map->data.size());

  size_t previous_matched = 0;
  size_t count_matched = 0;
  for (size_t i = 0; i < gt_map->data.size(); i++) {
    if (gt_map->data[i] == local_map->data[i]) {
      count_matched++;
    } else {
      EXPECT_EQ(local_map->data[i], -1);
    }
  }
