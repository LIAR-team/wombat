#include "wombat_srrg/srrg_solver/solver_core/sparse_block_matrix/sparse_block_matrix.h"
  ...
  float SparseBlockMatrix::symmetricProduct(const SparseBlockMatrix& x) const {
    ...
    for (int c = 0; c < num_block_cols_matrix; ++c) {
      ...
      for (auto it = col.begin(); it != col.end(); ++it) {
        ...
        // compute the final product x(r)^T * A(r,c) * x(c)
        tmp_block->matrixProduct(&sym_product, root_block);
        // handle off-diagonal blocks, in fact we represent a symmetric matrix by its upper
