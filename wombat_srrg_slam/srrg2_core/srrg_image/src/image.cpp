...
namespace srrg2_core {
  ...
  bool ImageSize::operator==(const ImageSize& img_size) const
  {
    return (width == img_size.width && height == img_size.height);
  }
  ...
  bool ImageSize::operator==(const cv::Size& cv_size) const
  {
    return (width == static_cast<size_t>(cv_size.width) && height == static_cast<size_t>(cv_size.height));
  }    
