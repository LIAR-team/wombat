...
protected:
  ...
  void generate(PointVectorCloudType_& points_in_camera_frame_, PointVectorCloudType_& features_) {
    ...
    points_in_camera_frame_[0].coordinates() =
      typename PointVectorCloudType_::PointType::VectorType(-0.5f, -0.5, 1.0); // top left
    points_in_camera_frame_[1].coordinates() =
      typename PointVectorCloudType_::PointType::VectorType(0.0f, 0.0, 5.0); // camera center
    points_in_camera_frame_[2].coordinates() =
      typename PointVectorCloudType_::PointType::VectorType(0.5, 0.5, 1.0); // bottom right

    // features to unproject in camera frame: image coordinates + depth
    features_.resize(3);
...
TEST_F(Point3fUnprojector, SparseUnprojectionPinholePointNormal3f) {
  ...
  for (size_t i = 0; i < points_in_camera_frame.size(); ++i) {
    ...
    // must be equal, we did not manipulate the float
...
TEST_F(Point3fUnprojector, SparseUnprojectionPinholePointIntensityDescriptor3f) {
  ...
  // populate intensity and descriptor fields in features
  // the field values must be carried along when unprojecting
  for (size_t i = 0; i < features.size(); ++i) {
  ...
  for (size_t i = 0; i < points_in_camera_frame.size(); ++i) {
    ...
    // must be equal, we did not manipulate the floats
