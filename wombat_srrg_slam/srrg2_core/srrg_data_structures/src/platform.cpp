...
namespace srrg2_core {
  ...
  bool Platform::addLink(Link* link_) {
    if (!link_) {
      DEBUG_VAR(platform_debug) << "Platform::addLink|WARNING: received null joint object" << std::endl;
      return false;
    }
  ...
  bool Platform::setup() {
    ...
    _root = 0;
    DEBUG_VAR(platform_debug) << "Platform::setup|updating hierarchy" << std::endl;
    _updateHierarchy();
