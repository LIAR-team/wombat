...
protected:
  void SetUp() override
  {
    // generate range and intensity measurements for a circle measurement
    ...
    // populate laser message with synthetic data
  ...
  const float angle_increment         = 0.02; // 100 measurements per sweep
...
protected:
  ...
  void loadDataset(
    const std::string & filepath_dataset_,
    ...
    const size_t & number_of_message_to_start_      = 0)
  {
    ...
    // load dataset from local path (provided by CI unittest environment)
    ...
    // grab first message to set laser message for configuration
