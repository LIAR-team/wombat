...
{
  // load the first 100 message packs of the killian dataset
  ...
  ASSERT_EQ(slammer->graph()->variables().size(), 11u);
  ASSERT_EQ(slammer->graph()->factors().size(), 10u);

  // run ground truth error computation
  benchamin->compute();
}

TEST_F(AIS, KillianCourtSLAMLoopClosure)
{
  // TODO need a tight circle with few messages (DEBUG speed)
