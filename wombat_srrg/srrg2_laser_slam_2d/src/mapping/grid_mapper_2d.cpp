#include "wombat_srrg/srrg2_laser_slam_2d/mapping/grid_mapper_2d.h"
...
#include <wombat_srrg/srrg_data_structures/traverse_line.h>
...
bool GridMapper2D::handleLocalMapMessage(BaseSensorMessagePtr msg_)
{
  ...
  if (!v) {
    ...
    grid_map = std::make_shared<GridMap2D>();
    ...
    new PropertyGridMap("grid_map_2d",
                        ...
                        grid_map);
    PropertyFrequencyGridType* fgrid = new PropertyFrequencyGridType(
      "frequency",
      "",
      grid_map.get(),
      FrequencyGridType());
    ...
    PropertyGridMap* grid_map_prop=lmap->dynamic_properties.property<PropertyGridMap>("grid_map_2d");
    grid_map=grid_map_prop->value();
...
bool GridMapper2D::handleLaserMessage(BaseSensorMessagePtr  msg_)
{
  ...
  if (platform() && !platform()->getTransform(
        laser_on_robot,
...
void GridMapper2D::processPendingMessages(double current_timestamp)
{
  ...
  for (auto it=_pending_maps.begin(); it!=_pending_maps.end(); ++it) {
    bool erase = handlePendingLocalMapUpdate(*(it->second), it->first);
    if (erase) {
      auto erased=it;
      ++it;
...
GridMapper2D::LaserMessageWithTfPtr GridMapper2D::findScanInPending(
...
void GridMapper2D::integrateScan(
  GridMap2D & grid_map,
  ...
  LaserMessagePtr scan)
{
  ...
  for (size_t i = 0; i < scan->ranges.size(); ++i) {
    float range = scan->ranges.value(i);
    if (range < scan->range_min.value()) {
      continue;
    }
    if (range > scan->range_max.value()) {
      continue;
    }
    bool draw_endpoint = true;
    if (range > param_usable_range.value()) {
      if (param_max_range_invalid.value()) {
        continue;
      }
      draw_endpoint = false;
      range = param_usable_range.value();
    }
    if (range < param_min_range.value()) {
      draw_endpoint = false;
      range = param_min_range.value();
    }

    float alpha = scan->angle_min.value()+scan->angle_increment.value()*i;
    float cos_a = cos(alpha);
    float sin_a = sin(alpha);
    Eigen::Vector2f local_endpoint(range * cos_a, range * sin_a);
...
void GridMapper2D::pasteLocalMap(GridMap2DPtr grid_map)
{
  ...
  PropertyFrequencyGridType* dest_grid_prop = _global_grid_map->property<PropertyFrequencyGridType>("frequency");
  if (!dest_grid_prop) {
    throw std::runtime_error("cast error, the proprerty is not a frequency grid");
  }

  FrequencyGridType& fmap = fgrid_prop->value();
  FrequencyGridType& dest_map = dest_grid_prop->value();
  Eigen::Matrix<float, 2,4>  corners;
  grid_map->cornersInGlobal(corners);
  ...
  for (auto it = contour.begin(); it != contour.end(); ++it) {
    ...
    if (r > current_row) {
      for (int cc = min_col; cc <= max_col; ++cc) {
        ...
        dest_map.at(r,cc)+=fmap.at(src_idx);
      }
...
{
  _lower_left=Eigen::Vector2f(
    std::numeric_limits<float>::max(),
    std::numeric_limits<float>::max());
  _upper_right=Eigen::Vector2f(
    -std::numeric_limits<float>::max(),
    -std::numeric_limits<float>::max());
...
bool GridMapper2D::cmdSaveMap(std::string & response, const std::string & file_prefix)
{
  ...
  if (! fgrid) {
      fgrid = new PropertyFrequencyGridType(
        "frequency",
        "",
