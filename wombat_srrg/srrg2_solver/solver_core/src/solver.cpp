...
bool Solver::computeMarginalCovariance(
  MatrixBlockVector& covariance_matricies_,
...
void Solver::extractFisherInformationBlocks(
  MatrixBlockVector& information_matricies_,
  const VariablePairVector& variables_)
{
...
Solver::_extractFisherInformationBlock(
  bool& transposed,
...
bool Solver::lambdaRatio(float& lambda_min, float& lambda_max, VariableBase* v, int level_)
{
  ...
  for (auto f_it : _graph->factors(v)) {
    ...
    bool active = std::binary_search(
      _active_factors[_current_level].begin(),
