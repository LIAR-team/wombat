...
static void setDifferenceInPlace(
  QuotientGraph::NodePtrSet & target,
...
static int setDifferenceSum(
  const QuotientGraph::NodePtrSet & target,
  const QuotientGraph::NodePtrSet & subtracted)
{
  auto it_t = target.begin();
  auto it_s = subtracted.begin();
...
void QuotientGraph::Node::approxDegreeUpdate(
  QuotientGraph& graph,
  NodePtrSet& super_neighbors,
...
void QuotientGraph::Node::externalDegreeUpdate(
  QuotientGraph& graph,
  NodePtrSet& super_neighbors)
{
  for (NodePtr n : super_neighbors) {
