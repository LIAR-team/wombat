...
namespace srrg2_solver {
  ...
  struct SE3toPoint3PinholeBAInitializerRule : public FactorGraphInitializerRule_<VariablePoint3> {
    ...
    bool doInit(VariablePoint3* v) override {
      ...
      for (auto it = active_factors.begin(); it != active_factors.end(); ++it) {
        ...
        Matrix3f J=
          geometry3d::skew(d)
          * projection->estimate().block<3,3>(0,0)
          * inv_pose.linear();
        Vector3f point_in_robot=inv_pose*x;
        Vector3f e        = geometry3d::skew(d) *
          (projection->estimate().block<3,3>(0,0)*point_in_robot
  ...
  struct SE3toPoint3RectifiedStereoInitializerRule : public FactorGraphInitializerRule_<VariablePoint3> {
    ...
    bool doInit(VariablePoint3* v) override {
      ...
      for (auto f_it: _graph->FactorGraphInterface::factors(v)) {
        ...
        if (! this_factor->triangulate(point_estimate,
                                       this_factor->measurement(),
                                       *root_v,
                                       *projection_matrix,
                                       *sizes))
          return false;
  ...
  FactorGraphInitializer::FactorGraphInitializer() {
    ...
    _rules.push_back(FactorGraphInitializerRulePtr(new SE3toPoint3RectifiedStereoInitializerRule(this))); 
 }
