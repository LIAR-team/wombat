...
public:
  ...
  /**
    ...
    * @param fixed point in the fixed cloud
    */
  inline void setFixed(const PointNormal3f & fixed)
  {
    fixed_point  = &fixed.coordinates();
    fixed_normal = &fixed.normal();
  }

  /**
    * @brief set point from the moving cloud
    * @param moving point in the moving cloud corresponding to the fixed
    */
  inline void setMoving(const PointNormal3f & moving)
  {
    moving_point  = &moving.coordinates();
