...
bool SE3PosePointRectifiedStereoErrorFactor::triangulate(
  Vector3f& point_in_world,
  const Vector3f& m,
...
bool SE3PosePointRectifiedStereoErrorFactor::computePrediction(
  Vector3f& prediction,
  const VariableSE3QuaternionRight& pose,
...
void SE3PosePointRectifiedStereoErrorFactor::errorAndJacobian(bool error_only_)
{
  ...
  Vector3f prediction (xl *iz,
                        y *iz,
                        (xl-xr) *iz);
  if (! inside(prediction,image_size) ) {
    return;
  }
  _is_valid=true;
