#include "wombat_srrg/srrg_solver/variables_and_factors/types_projective/se3_rectified_stereo_projective_error_factor.h"
...
inline void SE3RectifiedStereoProjectiveErrorFactor::errorAndJacobian(bool error_only)
{
  ...
  // compute points in image plane (reprojections) - assuming rigid stereo!
  ...
  // compute error (we compute the vertical error only once, since we assume rectified cameras)
  // MUST BE COMPENSATED WITH DOUBLE Y WEIGHT IN OMEGA for consistency with regular stereo
  _e = Vector3f(point_in_image_left.x() - fixed(0),
                point_in_image_left.y() - fixed(1),
                point_in_image_right.x() - fixed(2));

  // factor contributes at this point
  _is_valid = true;
  ...
  // compute the jacobian of the transformation - with optional weighted translation
  Matrix3_6f jacobian_transform;
  jacobian_transform.setZero();
  if (_mean_disparity_pixels > 0) {
    // consider mean disparity for translation gradient - super empirical 
    // note that this coefficient is constant over all iterations and should be moved outside
    ...
    // regular projective translation gradient contribution
    jacobian_transform.block<3, 3>(0, 0).setIdentity();
  }
  jacobian_transform.block<3, 3>(0, 3) = -2 * geometry3d::skew(moving);

  // precompute TODO move constant part of transform (rotation on first block) to pose binding
  ...
  // jacobian parts of the homogeneous division: left
  ...
  // we compute only the contribution for the horizontal error: right
  ...
  // we have to compute the full block
  _J.block<2, 6>(0, 0) = jacobian_left * jacobian_camera_matrix_transform;

  // we only have to compute the horizontal block
...
//! error function and factor for plain 3D stereo point to plane measurements
inline void SE3RectifiedStereoProjectiveWithSensorErrorFactor::errorAndJacobian(
  bool error_only)
{
  assert(_image_dim.x() > 0);
  assert(_image_dim.y() > 0);
  ...
  if (_mean_disparity_pixels > 0) {
    // consider mean disparity for translation gradient - super empirical
    // note that this coefficient is constant over all iterations and should be moved
    // outside
    const float weight_translation =
      std::min(0.1f + disparity_pixels / _mean_disparity_pixels, 1.0f);
    jacobian_transform.block<3, 3>(0, 0) = weight_translation * Matrix3f::Identity();
