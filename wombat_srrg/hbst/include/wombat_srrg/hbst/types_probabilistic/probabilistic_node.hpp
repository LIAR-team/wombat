...
#include "wombat_srrg/hbst/types_probabilistic/probabilistic_matchable.hpp"
#include "wombat_srrg/hbst/types/binary_node.hpp"

namespace srrg_hbst
{

// Node template specialization for descriptors of type: CDescriptorBinaryProbabilistic
...
public:
  ...
  using Matchable = ProbabilisticMatchableType_;
  using Descriptor = typename Matchable::Descriptor;
  using MatchableVector = std::vector<Matchable *>;
  using precision = real_precision_;
  using Match = BinaryMatch<Matchable, precision>;
  using BitStatisticsVector = typename Matchable::BitStatisticsVector;
  ...
  // initialization is left to the subclass the default constructor is required, since we do not
  // want to trigger the automatic leaf spawning of the baseclass in a subclass
  ProbabilisticNode() = default;

  // destructor: nothing to do (the leafs will be freed by the tree)
  virtual ~ProbabilisticNode() = default;

  // implementing the new spawn leafs function to handle the modified descriptors
  virtual const bool spawnLeafs(const SplittingStrategy& train_mode_) override
  {
    // buffer number of descriptors
    const uint64_t number_of_matchables = this->matchables.size();

    // if there are at least 2 descriptors (minimal split)
    if (number_of_matchables < 2) {
      // not enough descriptors to split
      return false;
    }
    ...
    // for all descriptors in this node (use base class descriptor)
    for (const Matchable* matchable : this->matchables) {
      // reduce accumulated probabilities (cast to the active descriptor)
    ...
    for (uint32_t index_bit = 0; index_bit < Matchable::descriptor_size_bits; ++index_bit) {
      ...
      if (this->bit_mask[index_bit]) {
        ...
        // for all descriptors in this node
        for (const Matchable* matchable : this->matchables) {
    ...
    if (this->index_split_bit == -1) {
      return false;
    }
    // if best was found - we can spawn leaves
    ...
    if (this->number_of_on_bits_total <= 0 || this->partitioning <= 0.5) {
      // split failed
      return false;
    }
    // enabled
    ...
    // first we have to split the descriptors by the found index - preallocate vectors
    ...
    // loop over all descriptors and assing them to the new vectors
    for (Matchable* matchable : this->matchables) {
      // check if split bit is set
...
protected:
  // only internally called: without split order
  ProbabilisticNode(
