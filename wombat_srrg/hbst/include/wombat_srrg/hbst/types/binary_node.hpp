...
#include "wombat_srrg/hbst/types/binary_match.hpp"
...
namespace srrg_hbst {
  ...
  public:
    // access only through this constructor: no mask provided
    BinaryNode(
    ...
    // access only through this constructor: mask provided
    BinaryNode(
      const MatchableVector& matchables_,
    ...
    // the default constructor is triggered by subclasses - the responsibility of attribute
    // initialization is left to the subclass this is required, since we do not want to trigger
    // the automatic leaf spawning of the baseclass in a subclass
    BinaryNode() = default;

    // destructor: recursive destruction of child nodes (risky but readable)
    virtual ~BinaryNode()
    {
      delete left;
      delete right;
    ...
    virtual bool spawnLeafs(const SplittingStrategy& train_mode_)
    {
      ...
      switch (train_mode_) {
        case SplittingStrategy::SplitEven: {
          // we have to find the split for this node - scan all indices
          for (uint32_t bit_index = 0; bit_index < Matchable::descriptor_size_bits; ++bit_index) {
            // if this index is available in the mask
            if (bit_mask[bit_index]) {
              // temporary set bit count
              uint64_t number_of_set_bits = 0;

              // compute distance for this index (0.0 is perfect)
              ...
              if (partitioning_current < partitioning) {
                ...
                // finalize loop if maximum target is reached
                if (partitioning == 0)
                  break;
              }
            }
      ...
      if (index_split_bit == -1 || partitioning >= maximum_partitioning) {
        // failed to spawn leaf - terminate recursion
        return false;
      }
      // if best was found and the partitioning is sufficient (0 to 0.5) - we can spawn leaves

      // get a mask copy
      Descriptor bit_mask_previous(bit_mask);

      // update mask for leafs
      bit_mask_previous[index_split_bit] = 0;

      // first we have to split the descriptors by the found index - preallocate vectors since
      ...
      // loop over all descriptors and assigning them to the new vectors based on bit status
      ...
      // this leaf becomes a regular node and hence does not carry matchables
      has_leafs = true;
      matchables.clear();
      _header.number_of_matchables_compressed = 0;

      // if there are elements for leaves
  ...
  public:
    ...
    // inner constructors (used for recursive tree building)
  protected:
    // only internally called: default for single matchables
    BinaryNode(
      Node* parent_,
      ...
      bit_mask(bit_mask_)
    {
      ...
      // recompute current number of contained merged matchables TODO make this less horribly
      // wasteful
  ...
  protected:
      ...
      uint64_t& number_of_set_bits_total_) const
    {
      ...
      // count set bits of all matchables in this node
      uint64_t number_of_set_bits = 0;
#ifdef SRRG_MERGE_DESCRIPTORS
      uint64_t number_of_set_bits_actual = 0;
      for (const Matchable* matchable : matchables_) {
        // accumulate set bit matchable counts
        if (matchable->descriptor[index_split_bit_]) {
          // make sure to weight merged matchables! default is 1, if not merged
          number_of_set_bits += matchable->number_of_objects;
          ++number_of_set_bits_actual;
        }
      }

      // this number is used to allocate for the partitioning
      // so we cannot count merged matchables twice here
      number_of_set_bits_total_ = number_of_set_bits_actual;
#else
      for (const Matchable* matchable : matchables_) {
        // just add the bits up (a set counts automatically as one)
  ...
  protected:
    ...
    // random number generator, used for random splitting (for all nodes)
