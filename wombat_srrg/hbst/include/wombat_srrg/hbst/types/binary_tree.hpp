...
#include "wombat_srrg/hbst/types/binary_node.hpp"

// helper macro for controlled reading and writing operations
...
public:
  ...
  using ScoreVector = std::vector<Score>;
  ...
  // ctor/dtor
public:
  // empty tree instantiation with specific identifier
  ...
  // construct tree upon allocation on filtered descriptors
  BinaryTree(
  ...
  // construct tree upon allocation on filtered descriptors: with bit mask
  BinaryTree(
    const uint64_t& identifier_,
    const MatchableVector & matchables_,
  ...
  // free all nodes in the tree without freeing the matchables - call clear(true)
  ~BinaryTree()
  {
    clear();
  }

  // shared pointer access wrappers
public:
  uint64_t
  getNumberOfMatches(
    const std::shared_ptr<const MatchableVector> matchables_query,
    uint32_t maximum_distance = 25) const
  {
    return getNumberOfMatches(*matchables_query, maximum_distance);
  }
  ...
  getMatchingRatio(
    ...
    uint32_t maximum_distance = 25) const
  {
    return getMatchingRatio(*matchables_query, maximum_distance);
  }

  uint64_t
  getNumberOfMatchesLazy(
    const std::shared_ptr<const MatchableVector> matchables_query,
    uint32_t maximum_distance = 25) const
  {
    return getNumberOfMatchesLazy(*matchables_query, maximum_distance);
  }

  const typename Node::real_type
  getMatchingRatioLazy(
    const std::shared_ptr<const MatchableVector> matchables_query,
    uint32_t maximum_distance = 25) const
  {
    return getMatchingRatioLazy(*matchables_query, maximum_distance);
  }

  const typename Node::real_type
  getMatchingRatioLazy(
    const MatchableVector& matchables_query,
    uint32_t maximum_distance = 25) const
  {
    return static_cast<typename Node::real_type>(
              getNumberOfMatchesLazy(matchables_query, maximum_distance)) /
            matchables_query.size();
  }
  ...
  {
    return static_cast<typename Node::real_type>(
              getNumberOfMatches(matchables_query, maximum_distance)) /
            matchables_query.size();
  }
  ...
  uint64_t getNumberOfMatches(const MatchableVector& matchables_query,
                                    uint32_t maximum_distance = 25) const {
    if (matchables_query.empty()) {
      return 0;
    }
    uint64_t number_of_matches = 0;

    // for each descriptor
    for (const Matchable * matchable_query : matchables_query) {
      // traverse tree to find this descriptor
      const Node* node_current = _root;
      while (node_current) {
        // if this node has leaves (is splittable)
        if (node_current->has_leafs) {
          // check the split bit and go deeper
          ...
          // check current descriptors in this node and exit
          for (const Matchable * matchable_reference : node_current->matchables) {
            if (maximum_distance > matchable_query->distance(matchable_reference)) {
              ++number_of_matches;
              break;
            }
          }
  ...
  const ScoreVector getScorePerImage(const MatchableVector& matchables_query,
    ...
    // identifier to vector index mapping - simultaneously initialize result vector
    ...
    for (const Matchable * matchable_query : matchables_query) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // check current descriptors for each reference image in this node and exit
          std::set<uint64_t> matched_references;
          for (const Matchable * matchable_reference : node_current->matchables) {
            if (matchable_query->distance(matchable_reference) < maximum_distance) {
#ifdef SRRG_MERGE_DESCRIPTORS
              ...
              const uint64_t& identifier_reference = matchable_reference->_image_identifier;
                ...
                // the query matchable can be matched only once to each reference image
    ...
    const real_type number_of_query_descriptors = matchables_query.size();
    for (Score& score : scores_per_image) {
      score.matching_ratio = score.number_of_matches / number_of_query_descriptors;
    }

    // if desired, sort in descending order by matching ratio
    if (sort_output) {
      std::sort(
  ...
  uint64_t getNumberOfMatchesLazy(const MatchableVector& matchables_query,
                                        uint32_t maximum_distance = 25) const {
    if (matchables_query.empty()) {
      return 0;
    }
    ...
    for (const Matchable * matchable_query : matchables_query) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // check current descriptors in this node and exit
          if (maximum_distance > matchable_query->distance(node_current->matchables.front())) {
            ++number_of_matches;
          }
          break;
        }
  ...
  // direct matching function on this tree
  void matchLazy(const MatchableVector& matchables_query,
  ...
  // direct matching function on this tree
  void match(const MatchableVector& matchables_query,
              MatchVector& matches_,
              uint32_t maximum_distance = 25) const {
    if (matchables_query.empty()) {
      return;
    ...
    for (const Matchable * matchable_query : matchables_query) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // current best (0 if none)
          const Matchable * matchable_reference_best = nullptr;
          uint32_t distance_best                    = maximum_distance;

          // check current descriptors in this node and exit
          for (const Matchable * matchable_reference : node_current->matchables) {
            uint32_t distance = matchable_query->distance(matchable_reference);
  ...
  getMatchesLazy(const std::shared_ptr<const MatchableVector> matchables_query,
                  uint32_t maximum_distance = 25) const {
    // wrap the call
    MatchVector matches;
    matchLazy(*matchables_query, matches, maximum_distance);
    return std::make_shared<const MatchVector>(matches);
  }

  //! @brief knn multi-matching function
  //! @param[in] matchables_query query matchables
  //! @param[out] matches_ output matching results: contains all available matches for all
  //! training images added to the tree
  //! @param[in] maximum_distance the maximum distance allowed for a positive match response
  void match(const MatchableVector& matchables_query,
              MatchVectorMap& matches_,
              uint32_t maximum_distance_matching_ = 25) const {
    if (matchables_query.empty() || _added_identifiers_train.empty()) {
      return;
    }

    // prepare match vector map for all iin the tree
    matches_.clear();
    for (const uint64_t identifier_tree : _added_identifiers_train) {
      matches_.insert(std::make_pair(identifier_tree, MatchVector()));

      // preallocate space to speed up match addition
      matches_.at(identifier_tree).reserve(matchables_query.size());
    }
  ...
  void add(const MatchableVector& matchables_,
    ...
    // prepare bookkeeping for training
    ...
    // train based on set matchables (no effect for do SplittingStrategy::DoNothing)
    train(train_mode_);
  }
  ...
  void train(const SplittingStrategy& train_mode_ = SplittingStrategy::SplitEven) {
    ...
    // check if we have to build an initial tree first (no training afterwards)
    if (!_root) {
    ...
    // nodes to update after the addition of matchables to leafs
    std::set<Node*> leafs_to_update;

#ifdef SRRG_MERGE_DESCRIPTORS
    // we need delayed insertion as we continuously scan the current references for merging
    _trainables.resize(_matchables_to_train.size());

    // matches to merge (descriptor distance == SRRG_MERGE_DESCRIPTORS)
    _merged_matchables.clear();
    _merged_matchables.reserve(_matchables_to_train.size());

    // currently we allow merging maximally once per reference matchable
    std::set<const Matchable *> merged_reference_matchables;
#endif

    // for each new descriptor - buffering new matchables and merging identical ones
    uint64_t index_new_matchable = 0;
    for (Matchable * matchable_to_insert : _matchables_to_train) {
      // traverse tree to find a leaf for this descriptor
      Node* node_current = _root;
      while (node_current) {
        // if this node has leaves (traversable)
        if (node_current->has_leafs) {
          // check the split bit and traverse the tree
          ...
          // we arrived in a leaf
#ifdef SRRG_MERGE_DESCRIPTORS
          bool insertion_required = true;

          // if we can absorb this matchable instead of having to insert it
          for (const Matchable * matchable_reference : node_current->matchables) {
            // if merge distance is satisfied
            // and this reference has not absorbed a matchable already in this call
          ...
          // if insertion is required - we could not merge the query matchable
          if (insertion_required) {
            // bookkeep matchable for addition
            // we need to bookkeep the node in order to add it to its matchables later on
...
#else
          // we can place the descriptor in the leaf on the spot
          node_current->matchables.push_back(matchable_to_insert);
          _matchables_to_train[index_new_matchable] = matchable_to_insert;
          ++index_new_matchable;
#endif
          // leaf always neeto be updated, merged or not
    ...
    for (MatchableMerge& mergable : _merged_matchables) {
      ...
      // free query (!) recall that the tree takes ownership of the matchables
      delete mergable.query;
    }
    _number_of_merged_matchables_last_training = _merged_matchables.size();
    _merged_matchables.clear();

    // insert matchables into nodes
...
#endif
    // check splits for touched leafs
  ...
  void matchAndAdd(const MatchableVector& matchables_,
                    MatchVectorMap& matches_,
                    uint32_t maximum_distance_matching_ = 25,
                    const SplittingStrategy& train_mode_      = SplittingStrategy::SplitEven) {
    if (matchables_.empty()) {
      return;
    }
    ...
    // check if we have to build an initial tree first
    if (!_root) {
    ...
    // prepare node/matchable list to integrate
    ...
    for (Matchable * matchable_query : matchables_) {
      ...
      while (node_current) {
        ...
        if (node_current->has_leafs) {
          ...
          // obtain best matches in the current leaf via brute-force search - bookkeeping
...
#ifdef SRRG_MERGE_DESCRIPTORS
          // if we can merge the query matchable into the reference
          if (matchable_reference &&
              merged_reference_matchables.count(matchable_reference) == 0) {
            assert(matchable_query->objects.size() == 1);

            // bookkeep matchable for merge
          ...
          // leaf neeto be updated, merged or not
          ++node_current->_header.number_of_matchables_uncompressed;
          leafs_to_update.insert(node_current);
          break;
        }
    ...
    // integrate new matchables: merge, add and spawn leaves if requested
    ...
    // insert new matchables and identifier
  ...
  // creates a matchable vector (pointers) from opencv descriptors - only available if OpenCV
  ...
  //! @brief clears complete structure (corresponto empty construction)
  void clear(bool delete_matchables_ = true)
  {
    // database identifier is not reset
  ...
  //! save complete database to disk
  bool write(const std::string & file_path) const
  {
    // open file (overwriting existing)
    ...
    // traverse complete tree to obtain leaf information
    ...
    // set endianness byte flag - when reading we will check for zero value
    const char endianness_check[] = {char(0)};
    GUARDED_IO(outfile, write, endianness_check, 1, "BinaryTree::write|ERROR: unable to write");

    // populate leaf count in header (currently only used for de/serialization)
    ...
    for (const Node* leaf : leafs) {
      ...
      // serialize matchables (descriptor) data
  ...
  bool read(const std::string & file_path)
  {
    ...
    // leaf buffer (static to allow easy escapes without requiring deallocation)
    // TODO use more compact data structure(s)
    ...
    // read leafs with matchable data
    ...
    for (size_t i = 0; i < _header.number_of_leafs; ++i) {
      ...
      // read split bit indices order - note that we also have to read the -1 of the leaf
    ...
    // after this point we use dynamic memory to build the tree - no exceptions are thrown!
    // assemble actual database by evaluating all leafs
    _root = new Node();
    ...
    for (size_t i = 0; i < leaf_headers.size(); ++i) {
      ...
      // grab any descriptor for evaluation decision rule
      // this is fine since all the descriptors reside in the same leaf and satisfy that rule
      const Descriptor& descriptor_sample = descriptors.back();

      // start from root for each leaf
      Node* current = _root;
      while (current) {
        // terminate if we reached a leaf
        if (bit_index_order[current->_header.depth] == -1) {
          ...
          // populate matchables of the leaf
          ...
          // otherwise it is always an intermediate node
          current->has_leafs = true;
        }

        // if this node has not been initialized yet
        ...
        // spawn leafs if necessary (we have a complete tree)
        if (!current->right) {
...
protected:
    ...
    std::map<uint64_t, Match> & best_matches_) const
  {
    ...
    // check current descriptors in this node
    for (const Matchable * matchable_reference : matchables_reference_) {
      // compute the descriptor distance
      uint32_t distance = matchable_query_->distance(matchable_reference);

      // if matching distance is within the threshold
      if (distance < maximum_distance_matching_) {
        // for every reference in this matchable
        for (const ObjectMapElement object : matchable_reference->objects) {
          const uint64_t& identifer_tree_reference = object.first;

          try {
            // update match if current is better than the current best - will jump to addition
            ...
            if (distance < best_distance_so_far) {
              ...
              // replace the best with this match on the spot - we don't have to update the
              // query information
            ...
            // if the match is equal to the last (multiple candidates)
            else if (distance == best_distance_so_far) {
              // add the candidate - we don't have to update the distance since its the same as
              // the best
    ...
    Matchable *& matchable_reference_for_merge_) const
  {
    ...
    for (const Matchable * matchable_reference : matchables_reference_) {
      ...
      if (distance < maximum_distance_matching_) {
        ...
        // if the matchable descriptors are identical - we can merge - note that
        ...
        if (distance <= maximum_distance_for_merge) {
          // behold the power of C++ (we want to keep the MatchableVector elements const)
    ...
    std::map<uint64_t, Match> & best_matches_) const
  {
    ...
    for (const Matchable * matchable_reference : matchables_reference_) {
      ...
      if (distance < maximum_distance_matching_) {
        ...
        try {
          // update match if current is better than the current best - will jump to addition if
          ...
          if (distance < best_distance_so_far) {
            ...
            // replace the best with this match on the spot - we don't have to update the query
            // information
            best_match_so_far.matchable_references =
              std::move(std::vector<const Matchable *>(1, matchable_reference));
            best_match_so_far.object_references =
  ...
  {
    // handle call on empty tree without node_ == _root
    if (_root == nullptr) {
      number_of_leafs_      = 0;
    ...
    // if we are in a node (that has leafs)
    if (node_->has_leafs) {
      assert(node_->right);
      assert(node_->left);

      // look deeper on left and right subtree
      ...
      // update statistics and terminate recursion
      ++number_of_leafs_;
