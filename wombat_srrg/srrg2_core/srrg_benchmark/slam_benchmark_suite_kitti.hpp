...
namespace srrg2_core {
  ...
  // 3D benchmark wrapper for: http://www.cvlibs.net/datasets/kitti/index.php
  // this dataset contains stereo vision and lidar data in SE(3)
  ...
  public:
    ...
    virtual void loadDatasetWithTF(const std::string& filepath_,
      ...
      // set up synchronizer for KITTI datasets
    ...
    BaseSensorMessagePtr getMessage() override {
      ...
      // load color and depth image from disk (buffered)
      // this is needed if we want to have buffered playback, otherwise the image links break
    ...
    void reset() override {
      // TODO implement proper reset playback
  ...
  protected:
    // we're loading messages on the spot as buffering image data consumes too much memory
