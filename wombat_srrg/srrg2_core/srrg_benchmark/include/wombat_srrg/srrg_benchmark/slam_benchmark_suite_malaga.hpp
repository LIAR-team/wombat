...
public:
  ...
  void loadGroundTruthFromBOSS(
    const std::string & filepath,
    const std::string & topic = "/ground_truth") override
  {
    ...
    if (!source->isOpen()) {
      throw std::runtime_error(
        "SLAMBenchmarkSuiteKITTI::loadGroundTruth|unable to load dataset: " + filepath);
    }
