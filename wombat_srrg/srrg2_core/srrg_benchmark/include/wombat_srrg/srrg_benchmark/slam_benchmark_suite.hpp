...
public:
  ...
  struct RelativeEstimateStamped
  {
    ...
    RelativeEstimateStamped(
      const EstimateType& estimate_source_in_target_,
  ...
  virtual ~SLAMBenchmarkSuite() = default;
  ...
  virtual void loadDatasetWithTF(
    const std::string & filepath,
  ...
  {
    (void)filepath;
    (void)platform_source_;
    (void)number_of_message_packs_to_read_;
    (void)number_of_message_pack_to_start_;
  }

  //! loads ground truth poses from file (s)
  virtual void loadGroundTruth(
    const std::string & filepath,
...
public:
  ...
  bool isRegression(
    const TranslationVectorType& maximum_mean_translation_rmse_,
...
protected:
  ...
  template <int Dimension_>
  bool _computeMeanAndStandardDeviation(
    const std::vector<Vector_<float, Dimension_>>& values_,
