...
public:
  ...
  virtual ~SLAMBenchmarkSuiteCARMEN() = default;
    ...
    const size_t & number_of_message_pack_to_start_ = 0) override
  {
    ...
    std::cerr << "SLAMBenchmarkSuiteCARMEN::loadDataset|buffering dataset: '" << filepath << "'"
              << std::endl;
    ...
    std::cerr << "SLAMBenchmarkSuiteCARMEN::loadDataset|loaded dataset: '" << filepath
  ...
  void writeTrajectoryToFile(const std::string & filename = "trajectory.txt") const override
  {
    (void)filename;
    // TODO implement me
  }
