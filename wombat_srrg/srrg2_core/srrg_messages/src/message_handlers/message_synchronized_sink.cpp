...
bool MessageSynchronizedSink::putMessage(BaseSensorMessagePtr msg_)
{
  ...
  for (size_t i = 0; i < param_topics.size(); ++i) {
    const std::string& t = param_topics.value(i);
    auto it_msg              = _message_map.find(t);
    assert(it_msg != _message_map.end() && "no messge in map");
    assert(it_msg->second && "invalid pointer");
    pack->messages.push_back(BaseSensorMessagePtr(it_msg->second));
    it_msg->second = 0;
  }
  ++_seq;
