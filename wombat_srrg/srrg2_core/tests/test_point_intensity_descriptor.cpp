...
TEST(PointDescriptorFieldCvMatBinary, DirectManipulation) {
  ...
  cv::Mat descriptor(1, 2, CV_8UC1); // 2 byte descriptor
  descriptor.setTo(0);
...
TEST(PointDescriptorFieldCvMatBinary, IndirectManipulation) {
  ...
  // perform base point operations that affect the coordinates field and the
  // descriptor
  ...
  // descriptors can not be "added" - currently the last value of the
  ...
  // descriptors can not be "subtracted" - currently the last value of the
  // expression is assigned ds TODO enable binary/mean floating point descriptor
  // merging ds TODO the current behavior is not intuitive!
  ASSERT_EQ(point_c.descriptor().at<uchar>(0, 0), 2);
  ASSERT_EQ(point_c.descriptor().at<uchar>(0, 1), 2);
...
TEST(PointDescriptorFieldCvMatBinary, HammingDistance) {
  ...
  descriptor_b.setTo(255); // 2x8=16 bit distance
  ...
  // soft comparison (works with any descriptor type and size)
...
TEST(PointDescriptorFieldBit256, ConversionCvToBitset) {
  ...
  descriptor_b.setTo(255); // set all byte blocks to the max

  // types to convert to each other
