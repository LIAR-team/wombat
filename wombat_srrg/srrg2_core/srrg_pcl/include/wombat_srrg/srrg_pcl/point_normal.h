...
#include "wombat_srrg/srrg_pcl/point_cloud.h"
#include "wombat_srrg/srrg_pcl/point_derived.h"
#include "wombat_srrg/srrg_pcl/point_ops.h"
...
namespace srrg2_core {
  ...
  struct PointNormal_
    ...
    CUDA_HOSTDEV PointNormal_() {
      BaseType::coordinates().setZero();
      normal().setZero();
    }
    CUDA_HOSTDEV PointNormal_(const BaseType& other) : BaseType(other) {
    }

    CUDA_HOSTDEV inline PointNormal_& operator=(const BaseType& other) {
      BaseType::operator=(other);
      return *this;
    }

    CUDA_HOSTDEV inline typename PointDirectionField::ValueType& normal() {
      return BaseType::template value<1>();
    }
    CUDA_HOSTDEV inline const typename PointDirectionField::ValueType& normal() const {
      return BaseType::template value<1>();
    }
  };
