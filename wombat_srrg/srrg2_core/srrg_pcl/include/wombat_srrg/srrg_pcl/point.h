...
namespace srrg2_core {
  ...
  struct Point_
    ...
    CUDA_HOSTDEV Point_(const BaseType& other) : BaseType(other) {
    }

    CUDA_HOSTDEV inline Point_& operator=(const BaseType& other) {
      BaseType::operator=(other);
      return *this;
    }

    CUDA_HOSTDEV inline typename PointCoordinatesField::ValueType& coordinates() {
      return BaseType::template value<0>();
    }
    CUDA_HOSTDEV inline const typename PointCoordinatesField::ValueType&
    coordinates() const {
      return BaseType::template value<0>();
    }
  };
