...
int32_t main() {
  ...
  // compute example transform
  Vector6d v; v << 1, 0, 0, 0.5, -0.5, 0.5;
  const Isometry3f example_transform(geometry3d::v2t(v));
  std::cerr << "---------------------------------------------------------- test transforms" << std::endl;

  // instanciate joints
  LinkWithJointPrismatic* j0 = new LinkWithJointPrismatic("j0", Vector3f(1, 0, 0), example_transform); // prismatic base
  Link* j1 = new Link("j1", example_transform); // fixed link
  Link* j2 = new Link("j2", example_transform); // fixed link
  Link* j3 = new Link("j3", example_transform); // fixed link
  Link* j4 = new Link("j4", example_transform); // fixed link
  Link* j5 = new Link("j5", example_transform); // fixed link
  Link* j6 = new Link("j6", example_transform); // fixed link
  Link* j7 = new Link("j7", example_transform); // fixed link
  Link* j8 = new Link("j8", example_transform); // fixed link
  ...
  // try duplicate additions
  platform.addLink(j8);
  platform.addLink(j4);
  std::cerr << "is well formed: " << platform.isWellFormed() << std::endl;

  // retrieve transforms of last state
  ...
  // try inserting another root joint
  ...
  // retrieve transforms at time: 1s
  ...
  // instanciate events for joint j0
  ...
  // add a transform event for link 1
  ...
  TransformEventPtr transform_event_t1(new TransformEvent(1, "j1", transform_update_t1, "j0")); // provide additional parent information (optional)
  Isometry3f transform_update_t2(Isometry3f::Identity());
  transform_update_t2.translation() = Vector3f(10, 5, 0);
  TransformEventPtr transform_event_t2(new TransformEvent(2, "j1", transform_update_t2, "j0")); // provide additional parent information (optional)
  TransformEventPtr transform_event_t3(new TransformEvent(3, "j1", transform_update_t2, "j0")); // an event which contains now new information, to check double insertion
