...
: public Aligner_<typename VariableType_::EstimateType,
...
public:
  ...
  virtual ~MultiAlignerBase_() = default;
  ...
  void setMovingInFixed(const EstimateType & moving_in_fixed) override;
...
protected:
  ...
  //! guess
  inline bool _computeCorrespondencesPerSlices(const EstimateType & moving_in_fixed)
  {
    ...
    for (size_t index_slice = 0; index_slice < number_of_slices; ++index_slice) {
      ...
      slice->setMovingInFixed(moving_in_fixed);
...
#include "multi_aligner_impl.hpp"
