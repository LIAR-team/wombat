...
void MultiRelocalizer_<SlamAlgorithmType_>::compute()
{
  ...
  if (!aligner) {
    std::cerr << FG_YELLOW(
              ...
              << std::endl;
    SRRG2_SLAM_DEBUG(relocalizer_debug) << "MultiRelocalizer_::compute|search: [\n";
    ...
    for (auto c : *(ThisType::_closure_candidates)) {
      ...
      SRRG2_SLAM_DEBUG(relocalizer_debug) << "(" << target_local_map->graphId();

      if (c->pose_in_target.translation().norm() > ThisType::param_max_translation.value()) {
        SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << FG_BRED("MAX_TRANSITION DROP: ")
      ...
      if (best_closure) {
        if (c->chi_inliers > best_closure->chi_inliers) {
          SRRG2_SLAM_DEBUG(relocalizer_debug)
            << ", " << FG_BRED("HIGH_CHI_INLIERS DROP: ") << c->chi_inliers << ")\n";
          continue;
        }
        if (c->num_correspondences < best_closure->num_correspondences) {
          SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << FG_BRED("LOW_MIN_CORRESPONDENCE DROP: ")
                                    << c->num_correspondences << ")\n";
          continue;
        }
      }
      best_closure = c;
      SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << FG_BGREEN("ACCEPT") << ")\n";
      else {
        std::cerr << FG_BGREEN("ACCEPT") << std::endl;
      }
    }
    SRRG2_SLAM_DEBUG(relocalizer_debug) << "]" << std::endl;
    ...
    for (auto c : *(ThisType::_closure_candidates)) {
      ...
      SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << num_correspondences;
      SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << num_inliers;
      SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << chi_inliers;

      // srrg this should be done in the slice
      if (num_inliers < param_relocalize_min_inliers.value()) {
        SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << FG_BRED("NUM_INLIERS DROP: ") << num_inliers << ")\n";
        continue;
      }
      ...
      SRRG2_SLAM_DEBUG(relocalizer_debug) << ", " << min_inliers_ratio;
