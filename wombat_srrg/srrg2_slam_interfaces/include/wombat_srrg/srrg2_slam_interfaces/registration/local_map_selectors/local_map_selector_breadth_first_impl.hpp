...
void LocalMapSelectorBreadthFirst_<SLAMAlgorithmType_>::compute()
{
  ...
  SRRG2_SLAM_DEBUG(loop_detector_closure_selector_debug) << "LocalMapSelectorBreadthFirst_::compute|search: "
  ...
  float max_cost = 0;
  // SRRG2_SLAM_DEBUG(loop_detector_closure_selector_debug) << "fixed set" << std::endl;
  // for (auto it:_current_local_map->dynamic_properties.properties()){
  //   SRRG2_SLAM_DEBUG(loop_detector_closure_selector_debug) << "p: " << it.first << std::endl;
  // }
  ...
  SRRG2_SLAM_DEBUG(loop_detector_closure_selector_debug) << " MAX_COST: " << max_cost << std::endl;
}
