...
#include "wombat_srrg/srrg2_slam_interfaces/initializers/initializer.h"
...
#include "wombat_srrg/srrg2_slam_interfaces/mapping/local_map_splitting_criterions/local_map_splitting_criterion_translation.hpp"
#include "wombat_srrg/srrg2_slam_interfaces/mapping/local_map_splitting_criterions/local_map_splitting_criterion_viewpoint.hpp"
#include "wombat_srrg/srrg2_slam_interfaces/mapping/local_map_splitting_criterions/local_map_splitting_criterion_visibility.hpp"
#include "wombat_srrg/srrg2_slam_interfaces/registration/loop_closure.h"
...
class MultiGraphSLAM_
: public srrg2_core::MessageSinkBase,
...
public:
  ...
  /**
    ...
    * @param[in] graph the factor graph
    */
  inline void setGraph(FactorGraphPtr graph)
  {
    _graph = graph;
  ...
  /**
    ...
    * @param[in] message the current message
    */
  bool putMessage(BaseSensorMessagePtr message) override;
  /**
    * @brief gather the given message
    * @param[in] message the current message
    */
  inline void setRawData(BaseSensorMessagePtr message)
  {
    assert(message);
    _message = message;
  }
  ...
  /**
    ...
    * @param[in] estimate robot pose in local map
    */
  inline void setRobotInLocalMap(const EstimateType & estimate)
  {
    _robot_in_local_map = estimate;
  }
  ...
  /**
    ...
    * @param[in] local_map current local map
    */
  inline void setCurrentLocalMap(LocalMapType * local_map)
  {
    assert(local_map);
    _current_local_map = local_map;
  }
...
protected:
  ...
  //! @param[in] reloc_closures set of relocalization closures to be validated
  void loopValidate(GraphItemPtrSet_<LoopClosureTypePtr> & reloc_closures);

  //! @brief optimizes the graph, if needed
  void optimize();

  //! @brief optimizes the graph, reads from _relocalize_closures
  //! @param[in] reloc_closures set of relocalization closures
  void relocalize(const GraphItemPtrSet_<LoopClosureTypePtr> & reloc_closures);

  //! @brief makes a new map around current position of the robot
  //! @param[in] info_scale information matrix scale factor
...
#include "multi_graph_slam_impl.hpp"
