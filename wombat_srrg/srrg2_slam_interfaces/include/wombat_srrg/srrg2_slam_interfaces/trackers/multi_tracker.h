...
#include "wombat_srrg/srrg2_slam_interfaces/registration/aligners/aligner.h"
#include "tracker.h"
#include "tracker_slice_processor_base.h"
#include <wombat_srrg/srrg_config/property_configurable_vector.h>
#include <wombat_srrg/srrg_solver/solver_core/solver_stats.h>
...
public:
  ...
  virtual ~MultiTrackerBase_() = default;

  /**
    * @brief provide the current message to the tracker
    * propagates the fresh sensor message to all tr-slices
    * @param[in] message: message or message pack for the tracker
    */
  void setRawData(srrg2_core::BaseSensorMessagePtr message) override;
  ...
  /**
    ...
    * the slice name)
    * @param[in] scene_: container of slices that holds the scene for each slice (local map)
    */
  ...
  /**
    ...
    */
  void setRobotInLocalMap(const EstimateType & robot_in_local_map) override;
...
#include "multi_tracker_impl.hpp"
