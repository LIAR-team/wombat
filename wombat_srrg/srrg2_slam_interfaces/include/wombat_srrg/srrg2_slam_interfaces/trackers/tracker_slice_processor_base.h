#pragma once
#include "wombat_srrg/srrg2_slam_interfaces/trackers/tracker_report.h"
#include "wombat_srrg/srrg2_slam_interfaces/raw_data_preprocessors/raw_data_preprocessor.h"
...
class TrackerSliceProcessorBase_
...
protected:
  /**
    ...
    * @param[in] robot_in_local_map: robot pose wrt map origin
    */
  virtual void setRobotInLocalMap(const EstimateType & robot_in_local_map);

  /**
    * @brief cache sensor pose wrt the robot
    * @param[in] sensor_in_robot: sensor in robot pose
    */
  inline void setSensorInRobot(const EstimateType & sensor_in_robot)
  {
    _sensor_in_robot = sensor_in_robot;
    _robot_in_sensor = _sensor_in_robot.inverse();
  }

  /**
    * @brief set current sensor data pack (will be processed here)
    * @param[in] msg: message or message pack for the tracker
    */
  virtual void setRawData(
    srrg2_core::BaseSensorMessagePtr msg,
    TrackerReportRecord & report_record) = 0;
  ...
  /**
    ...
    * @param[in] correspondences: precomputed correspondences from fixed to moving local map
    * @param[in] fixed_in_moving: relative transformation between the origins of the fixed map
    *                              (other map) wrt the moving map (current)
    * @param[in] robot_in_moving_local_map: pose of the robot in the current local map
    */
  virtual void setClosure(
    const srrg2_core::CorrespondenceVector & correspondences,
    const EstimateType & fixed_in_moving,
    const EstimateType & robot_in_moving_local_map)
  {
    (void)correspondences;
    (void)fixed_in_moving;
    (void)robot_in_moving_local_map;
  }

  /**
    * @brief set the global robot pose (for landmark estimation)
    * @param[in] robot_in_world: pose of the robot wrt the global map
    */
  void setRobotInWorld(const EstimateType & robot_in_world)
  {
    _robot_in_world = robot_in_world;
    _world_in_robot = _robot_in_world.inverse();
  }

  /**
    * @brief bind the post processed raw data (aka measurement) to a slice
    * @param[in] measurements scene
    */
  inline void bindMeasurements(srrg2_core::PropertyContainerDynamic& measurements)
  {
    _measurements_container = &measurements;
  }
  /**
    * @brief bind clipped scene slice for later reuse
    * @param[in] clipped scene
    */
  inline void bindClippedScene(srrg2_core::PropertyContainerDynamic& clipped_scene_container)
  {
    _clipped_scene_container = &clipped_scene_container;
  }
...
protected:
  ...
  void setRawData(
    srrg2_core::BaseSensorMessagePtr msg_,
...
#include "tracker_slice_processor_base_impl.hpp"
