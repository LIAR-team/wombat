...
#include "wombat_srrg/srrg2_slam_interfaces/trackers/tracker_slice_processor_prior.h"
...
void
MultiTrackerBase_<EstimateType_>::setRobotInLocalMap(
  const EstimateType_ & robot_in_local_map)
{
...
template <typename EstimateType_>
void MultiTrackerBase_<EstimateType_>::populateScene(
...
template <typename EstimateType_>
void
MultiTrackerBase_<EstimateType_>::setClosure(
  const CorrespondenceVector & correspondences,
  const EstimateType & reference_in_query,
...
template <typename EstimateType_>
void MultiTrackerBase_<EstimateType_>::setCurrentLocalMapId(
  srrg2_solver::VariableBase::Id clm)
{
  for (size_t i = 0; i < param_slice_processors.size(); ++i) {
    SliceProcessorTypePtr slice = param_slice_processors.value(i);
    assert(slice);
