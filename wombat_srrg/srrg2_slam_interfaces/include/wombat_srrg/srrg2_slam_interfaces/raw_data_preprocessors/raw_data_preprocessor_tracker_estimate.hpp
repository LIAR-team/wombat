...
public:
  ...
  //! dummy
  bool setRawData(
    srrg2_core::BaseSensorMessagePtr message,
    TrackerReportRecord& report) override
  {
    (void)message;
    (void)report;
    // unused
  ...
  //! @param[in] robot_in_local_map current tracker pose estimate w.r.t local map origin
  void setRobotInLocalMap(const EstimateType & robot_in_local_map)
  {
    _robot_in_local_map = robot_in_local_map;
  }

  //! recenter the current pose buffer behind the provided transform without addition
  //! @param[in] origin current tracker pose estimate w.r.t local map origin
  void setCoordinateFrameOrigin(const EstimateType & origin)
  {
