...
public:
  ...
  */
  virtual bool setRawData(
    srrg2_core::BaseSensorMessagePtr raw_data,
  ...
  */
  ...
  bool fillReport(
    TrackerReportRecord & report_record,
    srrg2_core::BaseSensorMessagePtr measurement)
  {
    TrackerInputHandlePtr handle(
      new TrackerInputHandle(
        measurement->topic.value(),
        measurement->seq.value(),
        measurement->timestamp.value()));

    report_record.measurements.push_back(handle);
    return true;
  }
...
std::shared_ptr<DestMessageType_> extractMessage(
  srrg2_core::BaseSensorMessagePtr measurement,
