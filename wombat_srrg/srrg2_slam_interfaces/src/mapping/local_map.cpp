...
void LocalMap2D::setDrawingReferenceFrame(
  ViewerCanvasPtr canvas,
  const EstimateType& pose_) const
{
  canvas->multMatrix(geometry3d::get3dFrom2dPose(pose_).matrix());
}

void LocalMap3D::setDrawingReferenceFrame(
  ViewerCanvasPtr canvas,
  const EstimateType& pose_) const
{
...
void initialize_local_map() __attribute__((constructor));
