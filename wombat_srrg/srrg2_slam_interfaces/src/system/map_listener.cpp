...
bool MapListener::handleLocalMapMessage2D(srrg2_core::BaseSensorMessagePtr msg_)
{
  ...
  for (auto it : lmap_msg->history) {
    ...
    trajectory->value()[timestamp]   = TrajectoryItem_<Isometry2f>(pose,lmap->graphId());
  }
  return true;
}
...
bool MapListener::handleLocalMapMessage3D(srrg2_core::BaseSensorMessagePtr msg_)
{
  ...
  for (auto it : lmap_msg->history) {
    ...
    const double& timestamp        = it->timestamp.value();
    trajectory->value()[timestamp]   = TrajectoryItem_<Isometry3f>(pose,lmap->graphId());
  }
  return true;
}
...
{
  NodeUpdateMessage3DPtr node_update_msg =
    std::dynamic_pointer_cast<NodeUpdateMessage3D>(msg_);
  if (!node_update_msg) {
    return false;
  }
